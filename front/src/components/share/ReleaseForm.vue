...
<template>
  <div class="release-form">
    ...
    <div v-if="type === 'releasePro'">
      <span @click="submit">发布项目</span>
    </div>
    <div v-if="type === 'showReleasePro'">
...
<script>
  ...
  export default {
    ...
    watch: {
      firstType () {
        let spanList = $('.select-first > span')
        spanList.removeClass('first-type-click')
        spanList.each((index, item) => {
          if(item.textContent === this.firstType){
            $(item).addClass('first-type-click')
            this.category = $(item)[0].getAttribute('data-type')
